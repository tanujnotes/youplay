...
public class TrendingVideoAdapter extends RecyclerView.Adapter<TrendingVideoAdapter.ItemViewHolder> {

    final String TAG = "TrendingVideoAdapter";
    private List<ItemModel.Item> playlistItems;
    private Activity activity;
    private OnLoadMoreListener loadMoreListener;
    private TrendingVideoAdapter.ItemViewHolder videoHolder;
    private boolean isLoading = false, isMoreDataAvailable = true;

    public TrendingVideoAdapter(Activity activity, List<ItemModel.Item> items) {
    ...
    @NonNull
    @Override
    public TrendingVideoAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemLayoutView = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_trending_video, null);
        return new TrendingVideoAdapter.ItemViewHolder(itemLayoutView);
    }

    @Override
    public void onBindViewHolder(@NonNull final TrendingVideoAdapter.ItemViewHolder holder, int position) {
        ...
        try {
            ...
            thumbnailUrl = playlistItems.get(position).getSnippet().getThumbnails().getStandard().getUrl();
        } catch (Exception e) {
            e.printStackTrace();
        }
